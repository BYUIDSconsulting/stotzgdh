...
gdh <- function(..., crop, crop_data) {
  ...
  if (any(is.na(c(...)))) {
    ...
    # Take the sum of the values, divide by 24, then subtract by the base temp value
    # Replace negative values with zero using pmax()
    GDD <- pmax((sum(values)/24)-base_temp,0)
    # Return GDD Value
    return(GDD)
  }
}
...
* The steps are listed below:
  ...
  4. For GDD, sum each GDH calculated during the day, divide that number by 24, and then subtract the low threshold temperature
