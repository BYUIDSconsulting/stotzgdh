...
* The steps are listed below:
  ...
  4. For GDD, sum each GDH heat units taken to find the total amount of heat absorbed by the plant
...
* crop name - this is taken from both the file imputed by the user earlier and `crop_base_temp.rda`
  - if the crop names in the imputed file does not match any of the crops found in `crop_base_temp.rda`, the package  will prompt the user to match the crop names not found in `crop_base_temp.rda` by showing the crop name that is not found and examples of crops that can be used and asks the user to correct thier imputed file. 
